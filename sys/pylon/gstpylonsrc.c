...
static gboolean
gst_pylonsrc_set_frameTransDelay (GstPylonSrc * src)
{
  GENAPIC_RESULT res;

  if (is_prop_implicit (src, PROP_FRAMETRANSDELAY)) {
    if (is_prop_set (src, PROP_FRAMETRANSDELAY)) {
      if (feature_supported (src, "GevSCFTD")) {
        GST_DEBUG_OBJECT (src, "Setting frame transmission delay to %d",
            src->frameTransDelay);
        res =
            PylonDeviceSetIntegerFeature (src->deviceHandle, "GevSCFTD",
            src->frameTransDelay);
        PYLONC_CHECK_ERROR (src, res);
      }
    }
    reset_prop (src, PROP_FRAMETRANSDELAY);
  }
  return TRUE;
...
static gboolean
gst_pylonsrc_set_bandwidthReserve (GstPylonSrc * src)
{
  GENAPIC_RESULT res;

  if (is_prop_implicit (src, PROP_BANDWIDTHRESERVE)) {
    if (is_prop_set (src, PROP_BANDWIDTHRESERVE)) {
      if (feature_supported (src, "GevSCBWR")) {
        GST_DEBUG_OBJECT (src, "Setting bandwidth reserve to %d",
            src->bandwidthReserve);
        res =
            PylonDeviceSetIntegerFeature (src->deviceHandle, "GevSCBWR",
            src->bandwidthReserve);
        PYLONC_CHECK_ERROR (src, res);
      }
    }
    reset_prop (src, PROP_BANDWIDTHRESERVE);
  }
  return TRUE;
